write_finemap_sumstats <- function(z,LD_file,k,prefix='data') {
  ...
  write(paste(cfg$z, cfg$ld, cfg$snp, cfg$config, cfg$k, cfg$log, length(z), sep=";"),
        file=cfg$meta,append=TRUE)
  return(cfg)
}

#' @param k prior. set to -999 to use default prior
finemap <- function(z,LD_file,sa=0.4,k=c(0,0,0,1),niter=1000000,prefix='data') {
  ...
  cfg = write_finemap_sumstats(z,LD_file,k,prefix)

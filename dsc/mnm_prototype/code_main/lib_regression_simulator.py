...
class RegressionData(dotdict):
    def __init__(self, X = None, Y = None, Z = None):
        ...
        self.debug = dotdict()
        self.x_mean = self.y_mean = self.z_mean = self.x_scale = None
        self.X = X
        self.Y = Y
        self.Z = Z
        self.xcorr = None
    ...
    def standardize_data(self):
        ...
        if self.X is not None and self.x_mean is None:
            ...
            self.x_scale = np.std(self.X, axis=0, ddof=0)
            self.X = scale(self.X)
...
class MultivariateMixture:
    ...
    def __init__(self, dim):
        ...
        self.coef = []
        self.grid = (0.1, 0.25, 0.4, 0.6)

    def set_pi0(self, pi0):
        self.pis['null'] = pi0

    def set_grid(self, grid):
        if isinstance(grid, float) or isinstance(grid, int):
            grid = (grid,)
        self.grid = grid
    ...
    def _init_canonical(self):
        ...
        equal_effects = 0.999
        self.Us['equal_effects'] = np.ones((self.R, self.R)) * equal_effects
        np.fill_diagonal(self.Us['equal_effects'], 1)
    ...
    def set_singleton(self, mode):
        ...
        if mode == 'singleton':
            ...
            # has to be the format of `singleton_*`
            self.pis[f"singleton_{mode.split('_')[1]}"] = 1 - self.pis['null']
        for k in self.Us:
            if k not in self.pis:
                self.pis[k] = 0
    ...
    def set_manual_blocks(self, n_groups = 2, group_het = [1,1], group_prop = [0.5,0.5]):
        '''
        creates mixtures of blocky patterns of sharing
        cf document: writeup/2019-05-30-12.57.00.pdf
        This will overwrite `self.Us`
        '''
        assert len(group_het) == n_groups
        assert len(group_prop) == n_groups
        groups = chunkIt(range(self.R), n_groups)
        for idx, item in enumerate(groups):
            self.Us[f'blocky_{idx+1}'] = np.zeros((self.R, self.R))
            self.Us[f'blocky_{idx+1}'][min(item):(max(item)+1), min(item):(max(item)+1)] = np.ones((len(item), len(item))) * group_het[idx]
            np.fill_diagonal(self.Us[f'blocky_{idx+1}'][min(item):(max(item)+1), min(item):(max(item)+1)], 1)
            self.pis[f'blocky_{idx+1}'] = group_prop[idx]

...
Create a table containing the GREGOR enrichment results for the paper:

```{r write-table}
dat1 <- subset(odds_ratios,method == "susie")
dat2 <- subset(odds_ratios,method == "mvsusie")
dat1 <- dat1[c("bed","trait","oddsratio","low","high","pvalue")]
dat2 <- dat2[c("oddsratio","low","high","pvalue")]
names(dat1) <-
  c("bed","trait","oddsratio.susie","low.susie","high.susie","pvalue.susie")
names(dat2) <-
  c("oddsratio.mvsusie","low.mvsusie","high.mvsusie","pvalue.mvsusie")
out_table <- cbind(dat1,dat2)
write.csv(out_table,"gregor.csv",quote = FALSE,row.names = FALSE)
```
...
p1 <- ggplot(subset(odds_ratios,trait == "crosstrait"),
  ...
  scale_x_continuous(limits = c(0,12),breaks = c(0,1,2.5,5,7.5,10)) +
...
```{r plot-all-traits, fig.height=6, fig.width=8}
p2 <- ggplot(odds_ratios,aes(x = oddsratio,y = bed,color = method)) +
  geom_point() +
  scale_color_manual(values = c("dodgerblue","red")) +
  scale_x_continuous(limits = c(0,34),breaks = c(0,1,10,20,30)) +
  geom_vline(xintercept = 1,linetype = "dotted") +
  labs(y = "") +
  theme_cowplot(font_size = 7) +
  facet_wrap(vars(trait),nrow = 3,ncol = 6) +
...
ggsave("../plots/gregor_global.eps",p1,height = 3.5,width = 4.5)
ggsave("../plots/gregor_by_trait.eps",p2,height = 5.5,width = 8)
```

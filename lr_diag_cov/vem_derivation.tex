...
Woodbury at every iteration. That is, we assume that
\begin{align}
\bs{V}_t = \bs{U}_t\bs{U}_t^{\intercal},
\end{align}
where $\bs{U}_t \in \mathbb{R}^{M \times L}$, where $L \ll M$. Then
\begin{align}
\left(\bs{V}_t^{-1} + \|\bs{x}_k\|^2\bs{\Lambda}\right)^{-1} = \|\bs{x}_k\|^{-2}\bs{\Lambda}^{-1} - \|\bs{x}_k\|^{-2}\bs{\Lambda}^{-1}\bs{U}_t\left(\bs{I}_L + \|\bs{x}_k\|^{-2}\bs{U}_t^{\intercal}\bs{\Lambda}^{-1}\bs{U}_t\right)^{-1}\bs{U}_t^{\intercal}\bs{\Lambda}^{-1}\|\bs{x}_k\|^{-2}.
\end{align}
Since $\bs{\Lambda}$ is diagonal, this requires much less computation
than calculating $\left(\bs{V}_t^{-1} +
  \|\bs{x}_k\|^2\bs{\Lambda}\right)^{-1}$ directly. To get these low
rank $\bs{V}_t$'s, you can use the \texttt{irlba} R package to
efficiently calculate the top SV's without having to calculate all of
the SV's.
